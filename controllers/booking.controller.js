...
const bookSlot = async (req, res) => {
    try {
        ...
        const interviewers = await Interviewer.find({
            availability: { $elemMatch: { day, date, times: time } }
        });

        if (interviewers.length === 0) {
            return res.status(404).json({ success: false, message: 'No interviewers available for this slot' });
        }
        ...
        const booking = await Booking.create({
            ...
            date,
            time,
            interviewer: selectedInterviewer._id,
            meetLink
        });

        // Optional: remove booked time from interviewer's availability
        selectedInterviewer.availability = selectedInterviewer.availability.map(slot => {
...
const getBookings = async (req, res) => {
    try {
        const bookings = await Booking.find().populate('interviewer', 'user availability');
...
const getUserBookings = async (req, res) => {
    try {
        ...
        const bookings = await Booking.find({ userId }).populate('interviewer', 'user availability');
...
module.exports = { bookSlot, getBookings, getUserBookings };